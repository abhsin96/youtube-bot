...
_VIDEO_ID = "vid1"
# EphemeralClient satisfies Pydantic's isinstance check against chromadb.ClientAPI.
# All _make_store calls are patched so no actual Chroma operations occur.
...
def test_build_retriever_returns_video_retriever():
    r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT)
...
def test_build_retriever_custom_k():
    r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, k=10)
    assert r.k == 10
...
def test_build_retriever_custom_score_threshold():
    r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, score_threshold=0.25)
...
def test_k_passed_to_similarity_search():
    ...
    with patch("src.retriever._make_store", return_value=store):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, k=7)
        r.invoke("query")
...
def test_threshold_filters_low_score_docs():
    ...
    with patch("src.retriever._make_store", return_value=_mock_store(pairs)):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, score_threshold=0.5)
...
def test_threshold_zero_returns_all_docs():
    ...
    with patch("src.retriever._make_store", return_value=_mock_store(pairs)):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, score_threshold=0.0)
        result = r.invoke("query")
    assert len(result) == 2
...
def test_threshold_one_returns_only_perfect_matches():
    ...
    with patch("src.retriever._make_store", return_value=_mock_store(pairs)):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, score_threshold=1.0)
        result = r.invoke("query")
    assert result == [_DOCS[0]]
...
def test_make_store_called_with_video_id_and_client():
...
def test_k_results_at_most_k_returned():
    ...
    pairs = [(_DOCS[0], 0.9), (_DOCS[1], 0.8)]
    with patch("src.retriever._make_store", return_value=_mock_store(pairs)):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, k=2)
        result = r.invoke("q")
    assert len(result) == 2
...
def test_k_is_forwarded_to_store_as_k1():
    store = _mock_store([(_DOCS[0], 0.9)])
    with patch("src.retriever._make_store", return_value=store):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, k=1)
        r.invoke("q")
...
def test_all_k_results_returned_when_all_pass_threshold():
    ...
    with patch("src.retriever._make_store", return_value=_mock_store(pairs)):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, k=3, score_threshold=0.5)
        result = r.invoke("q")
    assert len(result) == 3
...
def test_threshold_applied_after_sort():
    ...
    with patch("src.retriever._make_store", return_value=_mock_store(pairs)):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, score_threshold=0.3)
        result = r.invoke("q")
...
def test_empty_store_with_threshold_still_returns_empty():
    with patch("src.retriever._make_store", return_value=_mock_store([])):
        r = build_retriever(_VIDEO_ID, _fake_embeddings(), _CHROMA_CLIENT, score_threshold=0.9)
        result = r.invoke("q")
    assert result == []
